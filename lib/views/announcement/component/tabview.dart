...
class _AnnouncementTabViewState extends State<AnnouncementTabView> with AutomaticKeepAliveClientMixin{
  int current = 1;
  bool finished = false;
  final list = [].cast<item.Row>();
  Future<void> getAccouncementList() async{
    if (finished) {
      return;
    }
    final params = AccomounParams(
        row: 10, current: current, type: widget.data?.id
    );
            ...
            child: Column(
              children: [
                  ...
                  children: [
                    Expanded(
                      child: Padding(
                        ...
                        child: Text("${e.title}", style: context.textTheme.bodyMedium?.copyWith(
                          fontWeight: FontWeight.bold, overflow: TextOverflow.ellipsis
                        )),
                      ),
                    ),

                    Text("${e.publishTime}", style: context.textTheme.bodyMedium?.copyWith(
                ...
                Container(
                  ...
                  child: Text((e.title ?? "").nowrap(), maxLines: 3,style: context.textTheme.bodyMedium?.copyWith(
                      color: context.customTheme?.gray3, overflow: TextOverflow.ellipsis
                  )),
                )
              ],
            ),
          ).onTap(() {
            Get.toNamed("${AppRoutes.announcementDetail}/${e.id}?title=${(e.title ?? "").encode()}");
          });
        },
      ),
    );
